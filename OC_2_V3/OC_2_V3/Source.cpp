...
int main(){
	...
	if (str == "0"){
		for (int i = 0; i < n; i++){
			string q = to_string((double)(rand() % 10000) / (double)10000 + (double)(rand() % 10000));
			s += q + " ";
	...
	char lpszAppName[] = "c:\\Users\\User\\Documents\\Visual Studio 2013\\Projects\\OC_2_V3\\Release\\\Child.exe";
	STARTUPINFO si;
